<?php
    ...
    $id = $_GET['id'];
    $car = ['id'=>'','name'=>'','model'=>'','type'=>'Private car','price_per_day'=>'','availability_status'=>'available','image_path'=>'','description'=>''];
    if($id != ''){
        $result = getCarById($id);
        if(mysqli_num_rows($result) == 1){ $car = mysqli_fetch_assoc($result); }
    }
?>
...
<form action="../controllers/carController.php" method="post" enctype="multipart/form-data" onsubmit="return carValidation()">
    ...
    Name:<br><input type="text" name="name" id="name" value="<?php echo htmlspecialchars($car['name']); ?>"><br>
    Model:<br><input type="text" name="model" id="model" value="<?php echo htmlspecialchars($car['model']); ?>"><br>
    Type:<br>
    <select name="type">
    ...
    Description:<br><textarea name="description" id="description" rows="5"><?php echo htmlspecialchars($car['description']); ?></textarea><br>
